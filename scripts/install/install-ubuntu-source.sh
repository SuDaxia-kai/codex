...
usage() {
  cat <<'EOF'
Usage: scripts/install/install-ubuntu-source.sh [--install-path PATH] [--skip-backup]

Build the current repository's Rust CLI, back up the existing global codex binary,
and install the freshly built binary to a global path on Ubuntu/Debian.
...
Environment:
  ...
  CODEX_SKIP_PLATFORM_CHECK=1  Skip Linux/Ubuntu checks. Intended for testing only.
...
if [ "$SKIP_PLATFORM_CHECK" -ne 1 ]; then
  case "$(uname -s)" in
    Linux) ;;
    *)
      echo "This installer is for Ubuntu/Debian-style Linux systems." >&2
      exit 1
      ;;
  esac
  ...
  if [ -r /etc/os-release ]; then
    ...
    case "${ID:-}" in
      ubuntu|debian) ;;
      *)
        echo "Warning: this script was written for Ubuntu/Debian, detected ${ID:-unknown}." >&2
        ;;
    esac
  fi
fi
